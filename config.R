
descr_header_text <- "Description goes here"

choose_data_label_es <- "Elige datos"
choose_data_label_en <- "Choose data"
choose_data_choices_es <- c("Casos", "Altas", "Hospitalizados", "Fallecidos", "Activos")
choose_data_choices_en <- c("Cases", "Recovered", "Hospitalized", "Deaths", "Actives")

total_or_ccaa_label_es <- "Total o por CCAA"
total_or_ccaa_label_en <- "Total or per CCAA"
total_or_ccaa_choices_es <- c("Total", "Por CCAA")
total_or_ccaa_choices_en <- c("Total", "Per CCAA")

stat_label_es <- "Elige estadística"
stat_label_en <- "Choose statistics"
stat_choices_es <- c("Acumulado", "Cambio absoluto", "Cambio relativo")
stat_choices_en <- c("Cumulative", "Absolute change", "Relative change")

ccaa_label_es <- "Filtro CCAA"
ccaa_label_en <- "Filter CCAA"

choose_lan_label_es <- "Elige idioma"
choose_lan_choices_es <- c("ES", "EN")